...
import ZixunmaijiaModel from '../models/ZixunmaijiaModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let zixunwenti = req.query.zixunwenti
			if (zixunwenti) {

				if (zixunwenti.indexOf('%') != -1) {
					where.zixunwenti = {
						[Op.like]: zixunwenti
					}
				} else {
			...
			let zixunzhanghao = req.query.zixunzhanghao
			if (zixunzhanghao) {

				if (zixunzhanghao.indexOf('%') != -1) {
					where.zixunzhanghao = {
						[Op.like]: zixunzhanghao
					}
				} else {
					where.zixunzhanghao = {
						[Op.eq]: zixunzhanghao
			...
			let zixunxingming = req.query.zixunxingming
			if (zixunxingming) {

				if (zixunxingming.indexOf('%') != -1) {
					where.zixunxingming = {
						[Op.like]: zixunxingming
					}
				} else {
					where.zixunxingming = {
						[Op.eq]: zixunxingming
            ...
            if(sessionInfo['tableName'] == 'yonghu') {
                orwhere.push({ zixunzhanghao: sessionInfo['username'] })
            }
            if(orwhere.length > 0) {
                where[Op.or] = orwhere
            }
			...
			let result = await ZixunmaijiaModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await ZixunmaijiaModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await ZixunmaijiaModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ZixunmaijiaModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await ZixunmaijiaModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ZixunmaijiaModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM zixunmaijia WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "zixunmaijia"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM zixunmaijia " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("zixunmaijia" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM zixunmaijia " + where + " GROUP BY " + xColumnName + " DESC"
